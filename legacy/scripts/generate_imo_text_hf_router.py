#!/usr/bin/env python3
"""Generate IMO-AnswerBench responses through Hugging Face Router."""
...
DEFAULT_HF_ROUTER_MODEL = f"{DEFAULT_MODEL}:cheapest"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate IMO-AnswerBench text via Hugging Face Router.")
    parser.add_argument("--model", default=DEFAULT_HF_ROUTER_MODEL, help="Hugging Face Router model id.")
    parser.add_argument("--api-url", default=DEFAULT_API_URL)
    parser.add_argument("--api-key-file", type=Path, default=Path("api_tokens/.hf_token"))
    ...
    parser.add_argument("--generated-text-dir", type=Path, default=Path("outputs/imo-answerbench-text-hf-router"))
...
def hf_router_model_slug(model_id: str) -> str:
    return model_slug(f"HF-Router/{model_id}")
...
def call_with_retries(args: argparse.Namespace, api_key: str, prompt: str) -> dict:
    ...
    raise RuntimeError(f"Hugging Face Router request failed after retries: {last_error}") from last_error
...
def generate_one(args: argparse.Namespace, api_key: str, tokenizer, row: dict, row_number: int) -> tuple[str, Path, bool]:
    ...
    if not choices:
        raise RuntimeError(f"Hugging Face Router response had no choices for {row['Problem ID']}: {response}")
    ...
    text_path = save_imo_generation_bundle(
        ...
        model_id=f"HF-Router/{args.model}",
        prompt=prompt,
        result=result,
    )
...
def main() -> None:
    ...
    if not rows:
        print("All selected problems already have Hugging Face Router generation JSON files.", flush=True)
        return
    ...
    failures = []
    print(
        f"Generating {len(rows)} IMO-AnswerBench problem(s) via Hugging Face Router model {args.model} "
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="HF Router IMO", unit="problem"):
            row = futures[future]
            try:
